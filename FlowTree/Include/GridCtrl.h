...
//	LRESULT NotifyHandler(int /*idCtrl*/, LPNMHDR /*pnmh*/, BOOL& /*bHandled*/)
	...
	LRESULT OnCreate(UINT /*uMsg*/, WPARAM /*wParam*/, LPARAM /*lParam*/, BOOL& /*bHandled*/) {
		...
		CFontHandle hFont = GetParent().GetFont();
		if(hFont) {
			LOGFONT lf;
			hFont.GetLogFont(lf);
			...
			m_grid.m_font.CreateFont(12,0,0,0,FW_NORMAL,0,0,0,0,0,0,0,0,"MS Sans Serif");
			m_grid.m_font_bold.CreateFont(12,0,0,0,FW_BOLD,0,0,0,0,0,0,0,0,"MS Sans Serif");
		}
...
public:
	...
	long GetGridClientWidth() const {
		return m_grid.GetGridClientWidth();
	}

	bool IsEditing()
	{
		return m_grid.m_bEditing;
	}

	void EndEdit(bool bAbort) {
		m_grid.EndEdit(bAbort);
	}
...
protected:
	...
	public:
		CCellCheck(CColumn& column) : CCell(EDIT_CHECK,column), m_wndEdit() {
		}
		...
		virtual void Draw(CDCHandle dc,CRect& rc,const _variant_t& vtValue) {
			if(!IsNull(vtValue) && (long)vtValue) {
				dc.DrawFrameControl(&rc, DFC_MENU, DFCS_MENUCHECK);
			}
		}
