...
LRESULT CLogListView::OnRButtonDown(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL & bHandled)
{
  ...
  else if (nRet == ID_SHOW_CALL_IN_STUDIO)
  {
      Helpers::ShowInIDE(pNode, true);
  }
  else if (nRet == ID_SHOW_FUNC_IN_STUDIO)
  {
      Helpers::ShowInIDE(pNode, false);
  }
  else if (nRet == ID_TREE_COPY_INFO)
  {
	  ((FLOW_NODE*)pNode)->CopyFuncInfo();
  }
...
void CLogListView::SetColumns()
{
  ...
  if (gSettings.GetColPID() || gSettings.GetColThreadNN())
    AddColumn(_T("PID"), THREAD_COL);
...
void CLogListView::DrawSubItem(int iItem, int iSubItem, HDC hdc, RECT rcItem)
{
  ...
  if (col == ICON_COL)
  {
    ...
    //x = rcItem.left + ICON_SYNC * ICON_LEN;
    //ImageList_Draw(m_hListImageList, ICON_INDEX_SYNC, hdc, x, y, ILD_NORMAL);

    x = rcItem.left + ICON_BOOKMARK * ICON_LEN;
    LOG_NODE* pSyncNode = m_pView->selectedNode();
