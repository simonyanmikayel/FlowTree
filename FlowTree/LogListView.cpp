...
void CLogListView::UpdateCaret()
{
	...
	if (m_recCount)
	{
		...
		if (GetSubItemRect(curItem, 0, LVIR_LABEL, &itemRect))
		{
			...
			y = itemRect.top + (itemRect.bottom - itemRect.top) / 2 - gSettings.FontHeight() / 2;
...
LRESULT CLogListView::OnSetFocus(UINT /*uMsg*/, WPARAM /*wParam*/, LPARAM /*lParam*/, BOOL & bHandled)
{
	...
	if (!m_hasCaret)
	{
		CreateSolidCaret(1, gSettings.FontHeight());
		m_hasCaret = true;
		ShowCaret();
	}
	UpdateCaret();
...
void CLogListView::ApplySettings(bool fontChanged)
{
	...
	if (fontChanged)
	{
		...
		SelectObject(wndMemDC, gSettings.Font());
		SelectObject(itemMemDC, gSettings.Font());
	}
...
void CLogListView::SetColumns()
{
	...
	if (gSettings.ColPID() || gSettings.ColThreadNN())
		m_DetailType[m_cActualColumns++] = PID_COL;
	if (gSettings.ColTime())
		m_DetailType[m_cActualColumns++] = TIME_COL;
	if (gSettings.ColFunc())
...
void CLogListView::DrawSubItem(int iItem, int iSubItem, HDC hdc, RECT rcItem)
{
...

	{
		...
		if (pNode->isTrace())
		{
			gSettings.SetConsoleColor(((TRACE_NODE*)pNode)->MsgType, textColor, bkColor);
		}
